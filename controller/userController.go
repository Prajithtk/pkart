...
func UserSignUp(c *gin.Context) {
	...
	if uerr.Error == nil {
		c.JSON(http.StatusUnauthorized, "User Already exist")
		return
	}
...
func OtpSignUp(c *gin.Context) {
	...
	if oerr.Error != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "failed to fetch OTP"})
		return
	}
...
func ResendOtp(c *gin.Context) {
	...
	c.JSON(200, gin.H{"message": "otp resent successfully"})
}
...
func UserLogin(c *gin.Context) {
	...
	middleware.JwtToken(c, checkUser.ID, checkUser.Email, RoleUser)
...
func CartCheckOut(c *gin.Context)  {
	...
	database.DB.First(&address, "address_id=? AND user_id=?", addid, userId)
}